...

    {
       // Debug.Log(turnState);
		...
		if (turnState == attackturn.FIRST)
        {
            ...
            yield return new WaitForSecondsRealtime(waitTime);
			if (playerNumber == 1 || playerNumber == 3) {
				battleControl.GetComponent<BattleScript> ().p13 = 1;
			} else {
				battleControl.GetComponent<BattleScript> ().p24 = 1;
			}

			//if (!isDead || enemy.GetComponent<player> ().isDead != true)StartCoroutine(battleAnimation());
           
           
		} else if (battleControl.GetComponent<BattleScript>().turnstate == attackturn.SECOND && !isDead && enemy.GetComponent<player>().isDead != true)
		{	
		...
		else if (battleControl.GetComponent<BattleScript>().turnstate == attackturn.THIRD && !isDead && enemy.GetComponent<player>().isDead != true)
        {
            turnNmbr = 3;
            if (Control.third == 0)
            {
